...
using Eblcu.Bi.Tests.TestDatas;
...
namespace Eblcu.Bi.Tests
{
    ...
    /// Provides methods to easily work with <see cref="BiDbContext"/>.
    /// </summary>
    public abstract class AppTestBase : AbpIntegratedTestBase<BiTestModule>
    {
        ...
        {
            void NormalizeDbContext(BiDbContext context)
            {
        ...
        protected void UsingDbContext(Action<BiDbContext> action)
        {
            UsingDbContext(AbpSession.TenantId, action);
        }

        protected Task UsingDbContextAsync(Func<BiDbContext, Task> action)
        {
            return UsingDbContextAsync(AbpSession.TenantId, action);
        }

        protected T UsingDbContext<T>(Func<BiDbContext, T> func)
        {
            return UsingDbContext(AbpSession.TenantId, func);
        }

        protected Task<T> UsingDbContextAsync<T>(Func<BiDbContext, Task<T>> func)
        {
            return UsingDbContextAsync(AbpSession.TenantId, func);
        }

        protected void UsingDbContext(int? tenantId, Action<BiDbContext> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BiDbContext>())
                {
                    action(context);
                    context.SaveChanges();
                }
        ...
        protected async Task UsingDbContextAsync(int? tenantId, Func<BiDbContext, Task> action)
        {
            using (UsingTenantId(tenantId))
            {
                using (var context = LocalIocManager.Resolve<BiDbContext>())
        ...
        protected T UsingDbContext<T>(int? tenantId, Func<BiDbContext, T> func)
        {
            T result;
        ...
        protected async Task<T> UsingDbContextAsync<T>(int? tenantId, Func<BiDbContext, Task<T>> func)
        {
            T result;
